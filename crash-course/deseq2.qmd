...
::: {.callout-warning title="Challenge" collapse="true"}
Call back to our first few classes on how to inspect class and types of objects. Use these functions on the columns coerced to "factors". 

See if you can form your own opinions on what the "factor" class is internally and what the "levels" attribute means.

> Useful functions are `type()` and `attributes()`
:::
...
The `SummarizedExperiment` class has several "helper" functions to access data: `assay()`, `colData()`, `rowData()`
:::
...
::: {.callout-note title="Raw Counts"}
A DESeq Data Set requires "raw counts" as input. This means that they should not be normalized or transformed in any way. Generating a dds object with non-integer values should result in an error!
:::
